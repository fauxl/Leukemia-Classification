...
###NORMALIZZAZIONE###
leukemia_matrix <- as.matrix(Leukemia_GSEA)
normalized_matrix <- normalizeQuantiles(leukemia_matrix, ties=TRUE)
Leukemia_normalized <- as.data.frame(normalized_matrix)
Leukemia_normalized_T <- t(Leukemia_normalized)

###CALCOLO MATRICE DELLE DISTANZE POST-NORMALIZZAZIONE###
distance_matrix_normal <- dist(Leukemia_normalized_T)
class(distance_matrix_normal)
hc_norm <- hclust(distance_matrix_normal)
plot(hc_norm)
...
write.table(model_limma, file="model_limma.csv", quote = F, sep = ";", dec = ",", na = "", row.names = T)
colnames(model_limma) <- c("ALL", "AML")
fit <- lmFit(Leukemia_normalized, model_limma)
contrasts_names <- "ALL-AML"
contrasts <- makeContrasts(ALL-AML, levels= model_limma)
contrasts_fit <- contrasts.fit(fit, contrasts)
fit2 <- eBayes(contrasts_fit)
genes_names <- nrow(Leukemia_normalized)
p_table <- topTable(fit2, number = genes_names, coef = 1)
p_table$score <- sign(p_table$logFC)*-log10(p_table$P.Value)
p_table <- p_table[order(p_table$score, decreasing = TRUE),]
save(p_table, file="p_table.RData")
...
###ESTRAZIONE INDICI GENI CON SCORE > -0.118793330601436
indexes_g <- which(p_table[7] > -0.118793330601436)
p_table2 <- p_table[indexes_g,]

###CONFRONTO GENI OTTENUTI###
idx <- which(rownames(Leukemia_GSEA) %in% rownames(p_table2))
Leukemia_GSEA= Leukemia_GSEA[idx,]
Dataset_finale <- Dataset

rownames(Dataset) rownames(Dataset_finale)

idx2 <-which(rownames(Dataset_finale) %in% rownames(Leukemia_GSEA))

Dataset_finale <- Dataset_finale[idx2,]
save(Dataset_finale, file="FINAL_DATASET.RData")
write.table(Dataset_finale, file="Finale_Dataset.csv", quote = F, sep = ";", dec = ",", na = "", row.names = T)
...
train_y_1 <-train_x_1[1112,]
train_x_1 <- train_x_1[-1112,]

train_y_1 <-t(train_y_1)
train_x_1 <- t(train_x_1)

test_y_1 <- anal$Ana5[1112,] 
test_x_1 <- anal$Ana5[-1112,] 

test_y_1 <-t(test_y_1)
test_x_1 <- t(test_x_1)

train_x_2 <- cbind(anal$Ana1, anal$Ana2, anal$Ana3, anal$Ana5)
train_y_2 <-train_x_2[1112,]
train_x_2 <- train_x_2[-1112,]

train_y_2 <-t(train_y_2)
train_x_2 <- t(train_x_2)

test_y_2 <- anal$Ana4[1112,] 
test_x_2 <- anal$Ana4[-1112,] 

test_y_2 <-t(test_y_2)
test_x_2 <- t(test_x_2)

train_x_3 <- cbind(anal$Ana1, anal$Ana2, anal$Ana4, anal$Ana5)
train_y_3 <-train_x_3[1112,]
train_x_3 <- train_x_3[-1112,]

train_y_3 <-t(train_y_3)
train_x_3 <- t(train_x_3)

test_y_3 <- anal$Ana3[1112,] 
test_x_3 <- anal$Ana3[-1112,] 

test_y_3 <-t(test_y_3)
test_x_3 <- t(test_x_3)

train_x_4 <- cbind(anal$Ana1, anal$Ana3, anal$Ana4, anal$Ana5)
train_y_4 <-train_x_4[1112,]
train_x_4 <- train_x_4[-1112,]

train_y_4 <-t(train_y_4)
train_x_4 <- t(train_x_4)

test_y_4 <- anal$Ana2[1112,] 
test_x_4 <- anal$Ana2[-1112,] 

test_y_4 <-t(test_y_4)
test_x_4 <- t(test_x_4)

train_x_5 <- cbind(anal$Ana2, anal$Ana3, anal$Ana4, anal$Ana5)
train_y_5 <-train_x_5[1112,]
train_x_5 <- train_x_5[-1112,]

train_y_5 <-t(train_y_5)
train_x_5 <- t(train_x_5)

test_y_5 <- anal$Ana1[1112,] 
test_x_5 <- anal$Ana1[-1112,] 
...
model_norm_per_gene_1 %>%
  layer_dense(units=333, activation="relu", input_shape = c(1111)) %>%
...
Data_Name <- Dataset_finale_T[1:606,1112]

Dataset_finale_T[,1112] <- NULL
