...
save(Dataset, file="Dataset_outliernt.RData")
...
for(i in 1:10056) {
  ...
  for(j in 1:48) {
    ...
    if(is.na(Dataset_finale[i,j])){
      ...
      Dataset_finale[i,j] = (sum(Dataset_finale[i,1:48], na.rm=TRUE)/48)
...
library(ggbiplot)
typedata <- c(rep("AML",222),rep("ALL",150), rep("AML", 10), rep("ALL", 492), rep("AML",22), rep("ALL",29))
ggbiplot(Dataset.pca, ellipse=TRUE, circle = TRUE, obs.scale = 1, var.scale = 1,var.axes=FALSE, groups=typedata)

##########PRE-PROCESSING##########

#########RICERCA OUTLIER##########
string <-c(rep(0,10056))

Dataset_finale<-Dataset_normalized 
Dataset_finale_T<-Dataset_normalized_T

for(i in 1:48) {
  Outlier_removal <- Dataset_finale[1:10056,1:2]
  Outlier_removal[1:nrow(Dataset_finale),1] <- rownames(Dataset_finale)
  outlier_values <- boxplot(Dataset_finale[1:nrow(Dataset_finale),i], plot = FALSE)$out
...
Dataset<- Dataset_finale[-which(rownames(Dataset_finale) %in% string),]

###N. GENI 16836###

###VISUALIZZAZIONE GRAFICA OUTLIER###
plot(Dataset_finale,  xlim=c(0, 28),  main="Without Outliers", xlab="frequency", pch="*", col="red", cex=2)
OutVals <- boxplot(Dataset_finale)$out
...
labels <- c("ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "ALL", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML", "AML")
...
##########ESTRAZIONE INDICI GENI CON SCORE > -0.4917933############
indexes_g <- which(p_table[7] > -0.1054213)
p_table2 <- p_table[indexes_g,]

###CONFRONTO GENI INDIVIDUATI###
idx <- which(rownames(Dataset) %in% rownames(p_table2))
Dataset= Dataset[idx,]
...
Dataset_finale <- data
Dataset_finale <- as.data.frame(lapply(Dataset_finale, normalize_df))
...
labels <- c(rep("ALL",24),rep("AML",24))
binary_labels <- c(rep(0,24),rep(1,24))
...
train_y_1 <-train_x_1[172,]
train_x_1 <- train_x_1[-172,]

train_y_1 <-t(train_y_1)
train_x_1 <- t(train_x_1)

test_y_1 <- anal$Ana5[172,] 
test_x_1 <- anal$Ana5[-172,] 

test_y_1 <-t(test_y_1)
test_x_1 <- t(test_x_1)

train_x_2 <- cbind(anal$Ana1, anal$Ana2, anal$Ana3, anal$Ana5)
train_y_2 <-train_x_2[172,]
train_x_2 <- train_x_2[-172,]

train_y_2 <-t(train_y_2)
train_x_2 <- t(train_x_2)

test_y_2 <- anal$Ana4[172,] 
test_x_2 <- anal$Ana4[-172,] 

test_y_2 <-t(test_y_2)
test_x_2 <- t(test_x_2)

train_x_3 <- cbind(anal$Ana1, anal$Ana2, anal$Ana4, anal$Ana5)
train_y_3 <-train_x_3[172,]
train_x_3 <- train_x_3[-172,]

train_y_3 <-t(train_y_3)
train_x_3 <- t(train_x_3)

test_y_3 <- anal$Ana3[172,] 
test_x_3 <- anal$Ana3[-172,] 

test_y_3 <-t(test_y_3)
test_x_3 <- t(test_x_3)

train_x_4 <- cbind(anal$Ana1, anal$Ana3, anal$Ana4, anal$Ana5)
train_y_4 <-train_x_4[172,]
train_x_4 <- train_x_4[-172,]

train_y_4 <-t(train_y_4)
train_x_4 <- t(train_x_4)

test_y_4 <- anal$Ana2[172,] 
test_x_4 <- anal$Ana2[-172,] 

test_y_4 <-t(test_y_4)
test_x_4 <- t(test_x_4)

train_x_5 <- cbind(anal$Ana2, anal$Ana3, anal$Ana4, anal$Ana5)
train_y_5 <-train_x_5[172,]
train_x_5 <- train_x_5[-172,]

train_y_5 <-t(train_y_5)
train_x_5 <- t(train_x_5)

test_y_5 <- anal$Ana1[172,] 
test_x_5 <- anal$Ana1[-172,] 
...
model_norm_per_gene_1 %>%
  layer_dense(units=120, activation="relu", input_shape = c(171)) %>%
  layer_dropout(rate = 0.3) %>%
  layer_dense(units=84, activation="relu") %>%
  layer_dropout(rate = 0.3) %>%
  layer_dense(units=59, activation="relu") %>%
  layer_dropout(rate = 0.3) %>%
...
############ A) DATASET NORMALIZZATO PER CAMPIONE##################
...
Data_Name <- Dataset_finale_T[1:48,172]

Dataset_finale_T[,172] <- NULL
